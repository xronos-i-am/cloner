module Cloner::Postgres
  ...
  def pg_dump_remote
    ...
    do_ssh do |ssh|
      ...
      dump = pg_remote_auth + "pg_dump -Fc -U #{e ar_r_conf['username']} #{host} #{db_dump_options} #{e ar_r_conf['database']} > #{e(remote_dump_path + '/tmp.bak')}"
  ...
  def pg_dump_restore
    ...
    restore = pg_local_auth + "pg_restore --no-owner -Fc -c -U #{e ar_conf['username']} #{host} -d #{e ar_to} #{db_restore_options} #{e(pg_path + '/tmp.bak')}"
